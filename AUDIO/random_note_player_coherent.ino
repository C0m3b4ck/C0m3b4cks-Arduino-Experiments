//original from https://elegoo.com
...
void loop() {  
  thisNote = random(87);
  duration = random(100, 400);  //more standardized playback time (so that humans hear something else than static)
  
  tone(8, notes[thisNote], duration);  //plays random note with random duration from pin 8
  delay(duration * 1.3);  //slightly longer duration
  noTone(8);  //minimal silence gap
  delay(random(50, 200));  //random quiet pause for better coherence
}
